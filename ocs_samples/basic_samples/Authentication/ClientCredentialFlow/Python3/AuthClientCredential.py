...
class AuthClientCredential(object):
    """Handles communication with Sds Service"""

    def __init__(self, resource, clientId, clientSecret):
    ...
    def __getToken(self):
        ...
        if discoveryUrl.status_code < 200 or discoveryUrl.status_code >= 300:
            discoveryUrl.close()
            raise Error("Failed to get access token endpoint from discovery URL: {status}:{reason}".
