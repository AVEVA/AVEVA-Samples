...
declare function escape(s: string): string;
    ...
    export interface AdalEndpoint {
        endpointURL: string;
        endpointResourceURI: string;
    }
    ...
    export class AdalConfig {
        ...
        resource?: string;
        endpoints?: AdalEndpoint[];  // If you need to send CORS api requests.
        extraQueryParameter?: string;
        postLogoutRedirectUri?: string; // redirect url after succesful logout operation
        state?: string;
        expireOffsetSeconds?: any;
    ...
    export interface AdalLogging {
        log: (message: string) => void;
        level: AdalLoggingLevel;
    }

    export enum AdalLoggingLevel {
        ERROR = 0,
        WARNING = 1,
        INFO = 2,
        VERBOSE = 3
...
export class AuthenticationContext {
    ...
    _renewStates: any;
    _renewFailed: boolean;

    _singletonInstance: AuthenticationContext;
    ...
    constructor(private _config: AdalConfig) {
        ...
        if (_config.displayCall && typeof _config.displayCall !== 'function') {
        ...
        this.config = this._cloneConfig(_config);
    ...
    private _cloneConfig(obj: AdalConfig): AdalConfig {
    ...
    log(level, message, error) {
        ...
        if (level <= Logging.level) {
            ...
            if (this.config.correlationId) {
              formattedMessage = timestamp + ':' + this.config.correlationId + '-'
                + this._libVersion() + '-' + this.CONSTANTS.LEVEL_STRING_MAP[level] + ' ' + message;
            } else {
    ...
    _openPopup(urlNavigate, title, popUpWidth, popUpHeight) {
        try {
            ...
            */
            ...
            const popupWindow = window.open(urlNavigate, title, 'width='
    ...
    getCachedToken(resource) {
        this.info('getCachedToken for resource:' + resource);
        if (!this._hasResource(resource)) {
            return null;
        }

        const token = this._getItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + resource);
        const expiry: number = Number ( this._getItem(this.CONSTANTS.STORAGE.EXPIRATION_KEY + resource) );

        // If expiration is within offset, it will force renew
        const offset: number = this.config.expireOffsetSeconds || 300;

        if (expiry && (expiry > this._now() + offset)) {
            const secs = expiry - ( this._now() + offset );
            this.info('getCachedToken: time (seconds) left to expiry:' + (expiry - (this._now() + offset)) );
    ...
    registerCallback(expectedState, resource, callback) {
        ...
        if (!window['callBackMappedToRenewStates'][expectedState]) {
            window['callBackMappedToRenewStates'][expectedState] = function (errorDesc, token) {
                for (let i = 0; i < window['callBacksMappedToRenewStates'][expectedState].length; ++i) {
                    try {
                        window['callBacksMappedToRenewStates'][expectedState][i](errorDesc, token);
                    } catch (error) {
                        self.warn(error)
                    }
                }
    ...
    _renewToken(resource, callback) {
        ...
        const frameHandle = this._addAdalFrame('adalRenewFrame' + resource) as HTMLIFrameElement;
    ...
    _renewIdToken(callback) {
        // use iframe to try refresh token
        this.info('renewIDToken called:');
        this.info('renewIdToken is called');
        const frameHandle = this._addAdalFrame('adalIdTokenFrame') as HTMLIFrameElement;
    ...
    _loadFrameTimeout(urlNavigation, frameName, resource) {
        ...
        setTimeout(function () {
            if (self._getItem(self.CONSTANTS.STORAGE.RENEW_STATUS + resource) === self.CONSTANTS.TOKEN_RENEW_STATUS_IN_PROGRESS) {
                ...
                self.verbose('Loading frame has timed out after: ' +
                  (self.CONSTANTS.LOADFRAME_TIMEOUT / 1000) + ' seconds for resource ' + resource);
     ...
     */
    acquireToken(resource: string, callback: (err: string, u: string) => void) {
        this.info('AcquireToken for resource:' + resource);
    ...
    clearCache() {
        ...
        this._saveItem(this.CONSTANTS.STORAGE.ERROR, '');
        this._saveItem(this.CONSTANTS.STORAGE.ERROR_DESCRIPTION, '');
        const keys: string = this._getItem(this.CONSTANTS.STORAGE.TOKEN_KEYS);

        if (!this._isEmpty(keys)) {
            const keysarray: string[] = keys.split(this.CONSTANTS.RESOURCE_DELIMETER);
            for (let i = 0; i < keysarray.length; i++) {
                this._saveItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + keysarray[i], '');
                this._saveItem(this.CONSTANTS.STORAGE.EXPIRATION_KEY + keysarray[i], 0);
            }
        }
    ...
    getRequestInfo(hash) {
        ...
        if (parameters) {
            ...
            if (parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION) ||
                ...
                requestInfo.stateResponse = decodeURIComponent( stateResponse );
                ...
                if (!requestInfo.stateMatch && window.parent && window.parent['AuthenticationContext']) {
                    ...
                    for (let i = 0; i < statesInParentContext.length; i++) {
                        if (decodeURIComponent(statesInParentContext[i]) === decodeURIComponent(requestInfo.stateResponse)) {
    ...
    saveTokenFromHash(requestInfo) {
        ...
        if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
            this.info('Error :' + requestInfo.parameters.error + '; Error description:'
            ...
            if (requestInfo.stateMatch) {
                ...
                if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                    ...
                    this._saveItem(this.CONSTANTS.STORAGE.EXPIRATION_KEY
                      + resource, this._expiresIn(requestInfo.parameters[this.CONSTANTS.EXPIRES_IN]));
                }
                ...
                if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                    ...
                    if (this._user && this._user.profile) {
                        if (this._user.profile.nonce !== this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN)) {
                            ...
                            this._saveItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY
                              + resource, requestInfo.parameters[this.CONSTANTS.ID_TOKEN]);
                            this._saveItem(this.CONSTANTS.STORAGE.EXPIRATION_KEY + resource, this._user.profile.exp);
                        }
                    } else {
                        this._saveItem(this.CONSTANTS.STORAGE.ERROR, 'invalid id_token');
                        this._saveItem(this.CONSTANTS.STORAGE.ERROR_DESCRIPTION, 'Invalid id_token. id_token: '
    ...
    getResourceForEndpoint(endpoint: string): string {
        if (this.config && this.config.endpoints) {
            ...
            this.config.endpoints.some( s => {
                const configEndpoint = s;

                if (endpoint.indexOf(configEndpoint.endpointURL) > -1) {
                    resourceURI = configEndpoint.endpointResourceURI;
                    return true;
                }
    ...
    handleWindowCallback(hash) {
        ...
        if (this.isCallback(hash)) {
            ...
            if (!this.popUp) {// No need to redirect user in case of popup
              window.location.assign(this._getItem(this.CONSTANTS.STORAGE.LOGIN_REQUEST));
            }
        }
    };
    ...
    _getNavigateUrl(responseType, resource) {
        ...
        const urlNavigate = this.instance + tenant + '/oauth2/authorize'
          + this._serialize(responseType, this.config, resource) + this._addLibMetadata();
    ...
    _addAdalFrame(iframeId) {
        ...
        if (!adalFrame) {
            ...
            if (window.frames && window.frames[iframeId]) {
                // adalFrame = window.frames[iframeId] as HTMLElement;
                adalFrame = document.getElementById(iframeId);
            }
        }
