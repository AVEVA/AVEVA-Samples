...
myApp.controller('AppController', function AppController($scope, $http, $q) {
   ...
   */
  $scope.createDatabase = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      ...
      $scope.callURIText = 'GET request URI to retrieve the Asset Server: ' + callURI;
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            callURI = result[$scope.resultBody][$scope.resultLinks][$scope.resultSelf] + '/assetdatabases';
            ...
            return $scope.makePOSTRequest(callURI, httpOptions, requestBody, authType);
          })
          .then(function(result) {
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.createCategory = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/assetdatabases?path=\\\\' + assetServer + '\\' + $scope.databaseName;

      // Write the URI we're about to call to the textarea in the UI.
      $scope.callURIText = 'GET Request URI to retrieve the database: ' + callURI;
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            // Build the next URI for the POST request.
            callURI = result[$scope.resultBody][$scope.resultLinks][$scope.resultSelf] + '/elementcategories';
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.createTemplate = function(piWebAPIUrl, assetServer, piServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/assetdatabases?path=\\\\' + assetServer + '\\' + $scope.databaseName;
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            // Build the next URI for the POST request.
            callURI = result[$scope.resultBody][$scope.resultLinks][$scope.resultSelf] + '/elementtemplates';
          ...
          .then(function(result) {
            ...
            // Write the URI we're about to call to the textarea in the UI.
            callURI = piWebAPIUrl + '/elementtemplates?path=\\\\' + assetServer + '\\' + $scope.databaseName +
                  '\\ElementTemplates[' + $scope.templateName + ']';
          ...
          .then(function(machine) {
            callURI = machine[$scope.resultBody][$scope.resultLinks][$scope.resultSelf] + '/attributetemplates';

            // Write the URI we're about to call to the textarea in the UI.
            $scope.callURIText += '\nPOST Request URI to create an attribute: ' + callURI;
            $scope.callURIText += '\n\nPOST ' + $scope.templateName + ' attribute request body:\n' +
          JSON.stringify({Name: $scope.activeAttributeName, Description: '',
            IsConfigurationItem: true, Type: 'Boolean'}, null, 2);
          ...
          .then(function(result) {
            ...
            // Write the URI we're about to call to the textarea in the UI.
            $scope.callURIText += '\n\nPOST OSVersion attribute request body:\n' + JSON.stringify({Name: 'OSVersion',
              Description: 'Operating System Version', IsConfigurationItem: true, Type: 'String'}, null, 2);
          ...
          .then(function(result) {
            ...
            // Write the URI we're about to call to the textarea in the UI.
            $scope.callURIText += '\n\nPOST ' + $scope.sinusoidUAttributeName + ' attribute request body:\n' +
            JSON.stringify(
                {
            ...
            // Add Sinusoid U.
            return $scope.makePOSTRequest(callURI, httpOptions,
                {
                  Name: $scope.sinusoidUAttributeName,
                  Description: '',
                  IsConfigurationItem: false,
          ...
          .then(function(result) {
            $scope.codeResult += '\n' + $scope.sinusoidUAttributeName + ' attribute template created';

            //  Write the URI we're about to call to the textarea in the UI
            $scope.callURIText += '\n\nPOST ' + $scope.sinusoidAttributeName + ' attribute request body:\n' +
            JSON.stringify(
                {
          ...
          .then(function(result) {
            ...
            // Write the URI we're about to call to the textarea in the UI.
            $scope.callURIText += '\n\nPOST ' + $scope.sampleTagAttributeName + ' attribute request body:\n' +
            JSON.stringify(
                {
                  ...
                  Type: 'Double',
                  DataReferencePlugIn: 'PI Point',
                  ConfigString: '\\\\' + piServer + '\\%Element%_' + $scope.sampleTagName +
            ...
            return $scope.makePOSTRequest(
                ...
                httpOptions,
                {
                  Name: $scope.sampleTagAttributeName, Description: '',
                  IsConfigurationItem: false,
                  Type: 'Double',
                  DataReferencePlugIn: 'PI Point',
                  ConfigString: '\\\\' + piServer + '\\%Element%_' + $scope.sampleTagName +
          ...
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
            $scope.codeResult += '\n' + $scope.sampleTagAttributeName + ' attribute template created';
            return $scope.returnCode;
          })
          .catch(function(response) {
            $scope.returnCode = response.status;
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.createElement = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/assetdatabases?path=\\\\' + assetServer + '\\' + $scope.databaseName;
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            // Build the next URI for the POST request.
            callURI = result[$scope.resultBody][$scope.resultLinks][$scope.resultSelf] + '/elements';
          ...
          .then(function(result) {
            ...
            callURI = piWebAPIUrl + '/elements?path=\\\\' + assetServer + '\\' +
              $scope.databaseName + '\\' + $scope.elementName;
          ...
          .then(function(elementResponse) {
            callURI = piWebAPIUrl + '/elements/' + elementResponse[$scope.resultBody][$scope.resultWebId] + '/config';
            ...
            return $scope.makePOSTRequest(callURI, httpOptions, {includeChildElements: true}, authType);
          })
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.writeSingleValue = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    ...
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName +
        '\\' + $scope.elementName + '|' + $scope.sampleTagAttributeName;

      // Write the URI we're about to call to the textarea in the UI.
      $scope.callURIText = 'GET Request URI to retrieve the SampleTag: ' + callURI;
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            // Build the next URI for the POST request.
            callURI = result[$scope.resultBody][$scope.resultLinks][$scope.resultValue];
          ...
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
            $scope.codeResult = 'Attribute ' + $scope.sampleTagAttributeName + ' write value: ' + dataValue;
            return $scope.returnCode;
          })
          .catch(function(response) {
            $scope.returnCode = response.status;
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.writeSetOfValues = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName + '\\' +
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            // Build the next URI for the POST request.
            callURI = piWebAPIUrl + '/streams/' + result[$scope.resultBody][$scope.resultWebId] + '/recorded';
          ...
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
            $scope.codeResult = 'Attribute ' + $scope.sampleTagAttributeName + ' streamed 100 values: ';
            return $scope.returnCode;
          })
          .catch(function(response) {
            $scope.returnCode = response.status;
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.updateAttributeValue = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName + '\\' +
        $scope.elementName + '|' + $scope.activeAttributeName;

      // Write the URI we're about to call to the textarea in the UI.
      $scope.callURIText = 'GET Request URI to retrieve the ' + $scope.activeAttributeName + ' attribute: ' + callURI;
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            // Write the URI we're about to call to the textarea in the UI.
            $scope.callURIText += '\nPUT Request URI to update the ' + $scope.activeAttributeName +
              ' attribute value: ' + callURI;
            $scope.callURIText += '\n\nPUT request body:\n' + JSON.stringify(requestBody, null, 2);

            // Make a PUT request to write the dataset to the recorded streams.
            return $scope.makePUTRequest(callURI, httpOptions, requestBody, authType);
          })
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
            $scope.codeResult = 'Attribute ' + $scope.activeAttributeName + ' value set to true';
            return $scope.returnCode;
          })
          .catch(function(response) {
            $scope.returnCode = response.status;
   ...
   * @return {number} The return code of the last PI Web API request.
    */
  $scope.readSingleValue = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName + '\\' +
      ...
      // Write the URI we're about to call to the textarea in the UI.
      $scope.callURIText = 'GET Request URI to retrieve the ' + $scope.sampleTagAttributeName + ': ' + callURI;

      // Get the sample tag.
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            // Build the next URI for the POST request.
            callURI = piWebAPIUrl + '/streams/' + result[$scope.resultBody][$scope.resultWebId] + '/value';
          ...
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
            $scope.codeResult = $scope.sampleTagAttributeName + ' Snapshot Value: ' +
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.readSetOfValues = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName + '\\' +
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            // Build the next URI for the POST request.
            callURI = piWebAPIUrl + '/streams/' + result[$scope.resultBody][$scope.resultWebId] + '/recorded?startTime=*-2d';
          ...
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
            $scope.codeResult = $scope.sampleTagAttributeName + ' Values: ' + JSON.stringify(result, null, 2);
            return $scope.returnCode;
          })
          .catch(function(response) {
            $scope.returnCode = response.status;
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.reducePayloadWithSelectedFields = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName + '\\' +
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            // Build the next URI for the POST request.
            callURI = piWebAPIUrl + '/streams/' + result[$scope.resultBody][$scope.resultWebId] +
          ...
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
            $scope.codeResult = $scope.sampleTagAttributeName + ' Values with Selected Fields: ' +
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.doBatchCall = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName + '\\' +
      ...
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            ...
            const batchRequest = {
              1: {
                Method: 'GET',
                Resource: piWebAPIUrl + '/attributes?path=\\\\' + assetServer + '\\' + $scope.databaseName + '\\' +
              $scope.elementName + '|' + $scope.sampleTagAttributeName,
                Content: '{}',
              },
              2: {
            ...
            // Make a get request to retrieve the recorded values - restrict the response to only.
            // include the TimeStamp and Value fields.
            return $scope.makePOSTRequest(callURI, httpOptions, batchRequest, authType);
          })
          .then(function(result) {
            $scope.returnCode = result[$scope.returnStatus];
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.deleteElement = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/elements?path=\\\\' + assetServer + '\\' + $scope.databaseName +
        '\\' + $scope.elementName;

      // Write the URI we're about to call to the textarea in the UI.
      $scope.callURIText = 'GET Request URI to retrieve the AF element: ' + callURI;

      // Get the AF element.
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            // Build the next URI for the DELETE request.
            callURI = result[$scope.resultBody][$scope.resultLinks][$scope.resultSelf];
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.deleteTemplate = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/elementtemplates?path=\\\\' + assetServer + '\\' + $scope.databaseName +
        '\\ElementTemplates[' + $scope.templateName + ']';

      // Write the URI we're about to call to the textarea in the UI.
      $scope.callURIText = 'GET Request URI to retrieve the AF template: ' + callURI;

      // Get the AF template.
      return $scope.makeGETRequest(callURI, httpOptions, authType)
          .then(function(result) {
            // Build the next URI for the DELETE request.
            callURI = piWebAPIUrl + '/elementtemplates/' + result[$scope.resultBody][$scope.resultWebId];
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.deleteCategory = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/elementcategories?path=\\\\' + assetServer + '\\' + $scope.databaseName +
        '\\CategoriesElement[' + $scope.categoryName + ']';

      // Write the URI we're about to call to the textarea in the UI.
      $scope.callURIText = 'GET Request URI to retrieve the AF category: ' + callURI;
   ...
   * @return {number} The return code of the last PI Web API request.
   */
  $scope.deleteDatabase = function(piWebAPIUrl, assetServer, userName, userPassword, authType) {
    try {
      // Create the header and URI for the GET request.
      const httpOptions = $scope.callHeaders(userName, userPassword, authType);
      let callURI = piWebAPIUrl + '/assetdatabases?path=\\\\' + assetServer + '\\' + $scope.databaseName;
   ...
   * @return {Promise} The promise created by the http request.
   */
  $scope.makePOSTRequest = function(callURI, httpOptions, requestBody, authType) {
    return $http({
      method: 'POST',
      url: callURI,
      headers: httpOptions,
   ...
   * @return {Promise} The promise created by the http request.
   */
  $scope.makePUTRequest = function(callURI, httpOptions, requestBody, authType) {
    return $http({
      method: 'PUT',
      url: callURI,
      headers: httpOptions,
  ...
  $scope.onButtonClick = function() {
    ...
    switch ($scope.selectedCallOption) {
      case 'createDatabase': {
        returnCode = $scope.createDatabase($scope.piWebAPIUrl, $scope.assetServer, $scope.userName,
            $scope.userPassword, $scope.securityMethod);
      ...
      case 'createcategory': {
        $scope.createCategory($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'createtemplate': {
        $scope.createTemplate($scope.piWebAPIUrl, $scope.assetServer, $scope.piServerName, $scope.userName,
            $scope.userPassword, $scope.securityMethod);
        break;
      }
      case 'createelement': {
        $scope.createElement($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'deleteelement': {
        $scope.deleteElement($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'deletetemplate': {
        $scope.deleteTemplate($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'deletecategory': {
        $scope.deleteCategory($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'deletedatabase': {
        $scope.deleteDatabase($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'writesinglevalue': {
        $scope.writeSingleValue($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'writerecordedvalues': {
        $scope.writeSetOfValues($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'getsnapshotvalue': {
        $scope.readSingleValue($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'getrecordedvalues': {
        $scope.readSetOfValues($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'payloadselectedfields': {
        $scope.reducePayloadWithSelectedFields($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
      case 'batch': {
        $scope.doBatchCall($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword, $scope.securityMethod);
        break;
      }
      case 'updatevalue': {
        $scope.updateAttributeValue($scope.piWebAPIUrl, $scope.assetServer, $scope.userName, $scope.userPassword,
            $scope.securityMethod);
        break;
      }
